...
async function forwardRentalRequest(req, res, ivyPath) {
  if (!ivyApiKey) {
  ...
  try {
    ...
    console.error('Ivy rentals request failed:', error);
...
router.get('/', (req, res) => {
  forwardRentalRequest(req, res, '/v1/rentals');
});

router.get('/:listingId', (req, res) => {
  forwardRentalRequest(
    req,
    res,
    `/v1/rentals/${encodeURIComponent(req.params.listingId)}`
  );
});
